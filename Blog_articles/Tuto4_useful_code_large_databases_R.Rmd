...
#### 1.2.2 Merging the phytoplankton count together {.tabset}
...
for(i in seq_along(unique_name)) {
  ...
  i %in% as.numeric(unlist(strsplit(phyto_summ[which_match,"Row_matched_2"],"-")))
...
#### 1.2.2.2 Using the max = list(2,2,2,2) 
...
```{r new phytoplankton count, message=FALSE, warning=FALSE}
phyto_summ_3 <- NULL
unique_name  <-  seq_along(phyto_summ$Name)
for(i in seq_along(unique_name)) {
  which_match <- as.numeric(unlist(strsplit(phyto_summ[i,"Row_matched_2"],"-")))
...
### 2.1 Problem 
I use the <a href="https://dec.vermont.gov">Vermont DEC</a> plankton data, a monitoring that started in 2006. The 'visit dates' are entered heterogeneously. Not being able to easily access different years of the monitoring is a limitation.

```{r look at visit date format}
plktn$PlanktonData.VisitDate[seq(1,nrow(plktn),2000)]
```
