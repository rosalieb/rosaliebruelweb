...
I appreciate the functionality in RStudio that keeps in memory (through .RDataTemp) objects created at previous steps, even when closing RStudio. It is useful when running a script takes a while, and the output is useful but not necessarily *that* useful that I want to permanently save it to a local file.

However, on occasion, I have scripts I need to run once and that's it. At the end of said script, I would like to be able to remove any object that was created from the environment. In other words, I would like to be able to use rm(), but instead of doing it for the whole environment (\`ls()\`), I would like to do it for a specific script.
...
## `r (titlenb <- titlenb+1)`. Easy alternatives and why it did not fit my needs

### `r titlenb`.1. Alternative solution in the script directly
...
1.  If by mistake I forgot to run the first line \`initial_objects \<- ls()\`, it will be "too late" to go back.

2.  If by mistake I run the first line multiple times, after running the script a first time (for example, if I run everything above a certain line, or if I run "all chunks above" in a .Rmd).

3.  If there was already an element in the environment with the name. I frequently use names across scripts, for objects I will not keep (e.g., "temp" for a temporary object, "p1" for a plot, "out" for an output object that I would populate within a loop, etc.). If I already have an object "temp" in the environment, it won't be removed using the method above. However, it is not something I need to keep per my requirements stated in the section "problem".

### `r titlenb`.2. Alternative solution when sourcing a script  

A second option would be to run the script in its own environment, which is possible when sourcing a file. However, this would only work for a script I do not need to run line by line to visualize the output.
...
## `r (titlenb <- titlenb+1)`. Rapid overview of the function

The function is pretty simple, and follows three steps:

1.  Screening through the current script (although you could also set the path to another script if you want) with the function `getParseData()` from the library *utils*.
